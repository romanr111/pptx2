...
- №10 «Обґрунтування вибору кандидатів» (rejected[]; contact sheet
  відкладено) — **S**;
...
- сітка montage з Етапу 3 (P0 №4) не є передумовою жодного пункту тут;
  якщо колись зробимо відкладений contact sheet (№10), він
  перевикористає ту саму PIL-механіку сітки.
...
**Напрям фіксу — зменшити код, а не додати.** Первинна ідея була
навчити евристику розуміти колонки й сітки (нове поле `group`,
перевірка рівномірності кроку ±2%, збіг першої колонки між слайдами).
Це рух не в той бік: `check_role_alignment` фундаментально *вгадує намір
дизайнера* за координатами, і що складнішою стає здогадка, то більше в
ній припущень і крайових випадків. Наскрізне вирівнювання, ритм і
навмисність колонок — це судження про **montage** (Етап 3, P0 №4):
агент-рецензент бачить справжній зсув на зведеному кадрі краще за
будь-яку координатну евристику. Тому:

- **прибрати** deck-wide пул однакового `role` — саме він дає хибні
  спрацювання на багатоколонкових і процесних слайдах;
- **не додавати** ні поле `group`, ні логіку сітки/кроку — це код на
  здогадку, яку надійніше робить агент на montage.

### Кроки

1. `lint_deck.py`, `check_role_alignment` — звузити до дешевого,
   об'єктивного випадку: елементи одного `role` **у межах одного
   слайда**, яких більше одного і які за задумом стоять у стовпчик
   (наприклад, повторюваний `role: "body"` один під одним), мають
   ділити координату `x` у межах наявного толерансу (1% ширини).
   Кілька елементів одного `role`, рознесені по `x` (колонки), —
   **не** предмет цієї перевірки: різні `x` при близьких `y` читаються
   як навмисні колонки й пропускаються. Deck-wide порівняння «однакова
   `x` між слайдами» прибрати повністю.
2. Схему `spec.schema.json` **не чіпати** — поле `group` не вводиться.
3. Запустити `sync_agents.py` після правок.

Якщо після звуження перевірка вихолощується до майже-нічого — це
прийнятний результат: наскрізне вирівнювання перенесено на agent-review
montage, де воно й має жити. Ліпше тиха об'єктивна перевірка + agent
judgement, ніж голосна евристика-здогадка.

### Тести

- Unit-тести в `tests/test_qa_gates.py`:
  - стовпчик елементів одного `role` на одному слайді з реальним
    зсувом `x` понад толеранс — warn;
  - той самий стовпчик, вирівняний — без warn;
  - дві колонки (той самий `role`, рознесені `x`, близькі `y`) на
    одному слайді — **без** warn (навмисні колонки, не зсув).
...
Linter не скаржиться на навмисні колонки, картки та послідовності;
дешева об'єктивна перевірка стовпчика лишається, наскрізне вирівнювання
судить агент на montage.
...
**Критерій відбору перевірок:** додаємо лише те, що **об'єктивне й
машинно вимірюване** (ціле число, бінарний факт цілісності, назва шрифту
зі списку), і **пропускаємо все, що є візуальним судженням** — його
надійніше робить агент на montage/рендері, а детермінований поріг лише
шумить. Тому нижче — чотири перевірки, не шість.

### Кроки

Спершу — дві перевірки з найбільшою цінністю (об'єктивні, ловлять повний
провал доставки):

1. `check_package_integrity` (**error**): `zipfile.testzip()` + повторне
   відкриття python-pptx одразу після `apply_packaging` у
   `build_deck.py` — `native_packaging.py` редагує zip вручну, саме там
   ризик пошкодження пакета. Пошкоджений файл = repair dialog у
   PowerPoint = зірвана доставка; це найцінніша з перевірок і єдина
   error.
2. `check_font_substitution` (warn): `pdffonts` (poppler, вже у
   залежностях) по проміжному PDF із `render_all` у `render.py`; шрифт
   поза списком spec — warn. Підміна шрифту прямо б'є по головній вимозі
   клієнта — відповідності шаблону. Додати `pdffonts` до
   `check_external_tools` у `deck_qa.py` (зараз там `soffice`,
   `pdftoppm`, `textutil`).

Потім — дві дешеві об'єктивно-числові перевірки (нижчий пріоритет,
робити, якщо не роздувають обсяг):

3. `check_image_dpi` (`lint_render.py`, warn): для кожного
   image-елемента ефективний DPI = `px_width / (box.cx / 914400)`;
   warn нижче ~110 dpi при еталоні повного слайда 2560 px. Розміри —
   з `out/assets.json` або напряму через PIL. DPI — число, не судження.
4. `check_aspect_distortion` (`lint_render.py`, warn): лише для
...
**Свідомо НЕ додаємо** (це судження, а не метрики — їх робить агент на
рендері):

- `check_edge_mismatch` (кольорова відстань периметра зображення до фону
  слайда) — «чи видно шов / чужий фон?» надійніше бачить агент на
  повнорозмірному page-PNG; периметр-евристика хибно спрацьовує на
  зображеннях із легітимним контентом до краю. Замість перевірки —
  пункт у чеклісті agent-review montage (Етап 3).
- perceptual near-duplicates (dHash) — «це те саме зображення двічі?»
  агент помічає на montage, де всі слайди поруч. Наявна per-slide
  перевірка виміряних полів у `check_asset_duplication`
  (size/mode/alpha/dominant colors проти `assets.json`) лишається як є;
  новий перцептивний шар не додаємо — код на випадок, який agent-review
  і так покриває.

Далі:

5. Усі нові перевірки — advisory warn, окрім package integrity (error).
   Whitespace, hierarchy, balance, visual rhythm, density і shape count
   залишаються advisory (їх судить агент).
6. Після впровадження перегнати production-деку через `deck_qa.py` і
...
- Негативні fixtures у `specs/lint_test_fixtures/` на кожну
  перевірку (пошкоджений пакет, підмінений шрифт, низький DPI, stretch
  зі спотворенням), підключені до раннера №8 через
  `intentionally_broken` + expected check id.
- Unit-тести в `tests/test_qa_gates.py` для порогів і країв (contain
  проти stretch; DPI трохи вище/нижче порогу; package integrity на
  зіпсованому zip).
...
(Майстер-план явного критерію для №6 не дає; похідний.) Кожна з
чотирьох перевірок має позитивний і негативний тест; production-дека
проходить gate без нових errors; нові сигнали — advisory, окрім package
integrity (error). Візуальні судження (шов зображення, near-duplicate,
ритм) навмисно лишаються за agent-review montage, а не за детермінованим
порогом.

---

## №10 — обґрунтування вибору кандидатів — **S**

Briefs, provenance, licensing і AI-review gate вже існують;
`asset_resolver.py` — agent-operated (підкоманди `plan` / `import` /
`report`; web/gen-роботу виконує агент). Бракує зафіксованого
**обґрунтування вибору й відхилення** кандидатів.

**Розділення на дешеве-цінне і відкладене.** Майстер-план описував
generated contact sheet (PIL-сітка кандидатів у PNG). Але резолвер уже
agent-operated: агент **сам дивиться** на кандидатів під час вибору —
згенерований PNG потрібен людині для аудиту, а не агентові для рішення.
Тому цінне-й-дешеве (provenance-слід вибору) робимо зараз, а
PIL-генератор contact sheet — **відкладаємо**:

- зафіксований `rejected[]` у sidecar відповідає на аудиторське питання
  «чому саме це зображення?» без жодної PIL-механіки;
- каталог `out/briefs/<brief_id>/candidates/` з майстер-плану сьогодні
  **не існує** (`command_plan` повертає кандидатів лише JSON-записами;
  web-кандидатів агент кладе куди завгодно) — будувати PNG-генератор
  поверх неіснуючої конвенції = код перед потребою.

### Кроки

1. **Зараз (цінне, дешеве):** причини вибору/відхилення в provenance.
   `selected_rationale` уже обов'язкове поле sidecar (`command_import`,
   перевіряється `provenance_status`); додати `rejected[]`
   (`candidate`, `reason`) — повторюваний аргумент `--rejected` для
   `command_import` — і показувати його в `command_report`. Це
   робить вибір простежуваним у тому самому артефакті, що вже є.
2. **Відкладено (до реальної потреби):** PIL contact sheet
   (`out/briefs/<brief_id>_contact.png`) і конвенція каталогу
   `out/briefs/<brief_id>/candidates/`. Вводити разом — коли зʼявиться
   людина-аудитор, якій потрібен єдиний оглядовий кадр, і коли workflow
   у SKILL.md справді почне складати кандидатів у конвенційний каталог.
   Ту саму PIL-механіку сітки, що й montage (Етап 3), можна перевикорис­
   тати тоді без дублювання.
3. Порядок джерел незмінний: user-provided → template → approved
   project assets → web → AI generation лише після невдалого пошуку.
   `rejected[]` цей порядок не змінює — лише документує вибір.

### Тести

- `tests/test_asset_resolver.py`: `import` з `--rejected` → sidecar
  містить `rejected[]`; `report` його відображає.

### Критерій готовності

Кожне зовнішнє зображення має brief, provenance, licensing status,
verification notes і зафіксовану причину вибору (та відхилених
кандидатів). Згенерований contact sheet — відкладений до появи
людини-аудитора.
...
- **Відкладений contact sheet №10.** Свідомо робимо лише `rejected[]`
  provenance; PIL-генератор не будуємо, поки немає ні конвенційного
  каталогу кандидатів, ні людини-аудитора, якій потрібен оглядовий PNG.
  Ризик — що вибір лишиться непростежуваним; його знімає саме
  `rejected[]` у sidecar, а не згенерований аркуш.
